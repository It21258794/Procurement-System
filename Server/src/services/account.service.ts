...
async function findAccountByCatogory(catogory: string): Promise<any[]> {
  ...
  async function updateAccount(accountId: string, updatedData: any): Promise<any>{
    try {
        console.log(accountId);
        console.log(updatedData);
        const updatedItem = await Account.findByIdAndUpdate(accountId, updatedData, { new: true });
